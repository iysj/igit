	...
	return {
		...
		addTab:function(_title,_url){
			...
			if($(boxId).tabs('exists',_title) ){
				var pp = $(boxId).tabs('select',_title);
				if(pp.find('iframe').length > 0) {
					var refresh_iframe = pp.find('iframe')[0];
					refresh_iframe.contentWindow.location.href=_url;
				}
			}else{		
				...
				$(boxId).tabs('add',{
					    ...
					    tools:[{
					    	...
					    	handler: function() {
					    		var ttab = $(boxId).tabs('getSelected');
					    		$(boxId).tabs('update', {
					    			tab : ttab,
					    			options : ttab.panel('options').tab
					    		});
					    	}
					    }]
					});
		...
		modifyPwd:function(){
			...
			if(pwdForm.form('validate')){
				...
				easyui.saveForm(pwdForm,function(data){
